...
Next free port: 8065. Sidebar color order: purple→blue→green→amber→teal→rose→orange→lime→red.
...
7. Add BOTH a sidebar nav item and a home card to the homepage (keep them in sync).
   Tool links are plain <a> WITHOUT data-section so the SPA router leaves them alone.
